      ...
      <div className='' >
        ...
        <div className="p-4 my-3 w-[500px] ">
          <Link href={`/post/${post.slug}`}>
            <h3 className="font-extrabold  hover:underline underline-offset-4  text-2xl ">{post.title}</h3>
          </Link>

          <div className='flex gap-4 my-2 items-center '>
            <div className="whitespace-nowrap rounded-full bg-[#9147ff] px-2.5 text-[#fff] py-0.5 ">
              {post.Categories?.map((category) => (
                  <Link className="md:whitespace-nowrap rounded-full font-normal  text-sm  " key={category.id} href={`/[cSlug]`} as={`/${category.cSlug}`}>
                      {category.title}
                  </Link>
                ))}
            </div>
            ...
            <h4 className='text-sm  font-extralight'>By staff</h4>
              ...
              <p className="text-[12px] text-[#E90C59] font-normal "> 
                <time dateTime={post.date}>
                {isMoreThanADay ? date.tz('Europe/Berlin').format('ha z') : date.fromNow()} 
                </time>
              </p>
            </div>
          </div>
      ...
      const isMoreThanADay = moment().diff(date, 'days') >= 1;
        ...
        return (
          < >
            ...
            <div className="crd mx-auto lg:w-[75%]">
                  <div className="sub-crd category">
                    <span className="text_span flex items-center   space-x-6">
                    {post.Categories?.map((category) => (
                      <Link className="md:whitespace-nowrap rounded-full font-normal  text-white text-lg hover:underline  " key={category.id} href={`/[cSlug]`} as={`/${category.cSlug}`}>
                          {category.title}
                      </Link>
                    ))}
                  ...
                  <Link href={`/post/${post.slug}`} className='crd_container w-full mx-auto'>
                    <Image
                      src="https://i.postimg.cc/C5rqnSYp/6206720.jpg"
                      width={500}
                      height={20}
